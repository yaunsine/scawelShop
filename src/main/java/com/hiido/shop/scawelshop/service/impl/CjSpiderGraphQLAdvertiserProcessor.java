 ...
 */
@Service
public class CjSpiderGraphQLAdvertiserProcessor implements PageProcessor {
    ...
    private final Site site = Site.me().setSleepTime(2000).setRetryTimes(5).setTimeOut(180000)
            .setCharset("utf-8")
    ...
    private final Logger logger = LoggerFactory.getLogger(CjSpiderGraphQLAdvertiserProcessor.class);
    ...
    public void start() {
        ...
        for (Integer advertiserId: advertiserIds) {
            ...
            int max = 10000;
            /*
            if (advertiserId == 5176015) {
               continue;
            }
            advertiserId = 5178484; // 5178099,5178484
            删除原有的数据
            boolean delFlag = shoppingProductMapper.deleteShoppingProductsByAdvertiserId(advertiserId);
            if (delFlag) {
               logger.info("已删除原有的数据(advertiserId): " + advertiserId);
            }
            */
            if (advertiserId == 5178287) {
                // 5401824
                jumpFlag = false;
            }
            if (jumpFlag) {
